...
def parse_args():
    ...
    parser.add_argument('--name', default='colon_DPMNet_woDS',#CVC_ClinicDB_DPMNet_woDS,Kvasir-SEG_DPMNet_woDS
                        help='model name')
...
def main():
    ...
    print('models/%s/model.pth'%config['name'])
            ...
            for i in range(len(output)):
                for c in range(config['num_classes']):
                    cv2.imwrite(os.path.join('outputs', config['name'], str(c), meta['img_id'][i] + '.png'),
                                (output[i, c] * 255).astype('uint8'))#.jpg改为png
